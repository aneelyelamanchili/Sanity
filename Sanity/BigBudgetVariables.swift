...
//    required convenience init?(coder decoder: NSCoder)
    ...
    class func getData()
    {
        ...
        do
        {
            budgetArray = try context.fetch(MyBigBudget.fetchRequest())
        }
        catch
        {
    ...
    {
        for i in 0...BigBudgetVariables.budgetArray.count - 1
        {
            if BigBudgetVariables.budgetArray[i].historyArray.isEmpty == false
            {
                return false
            }
        }
    ...
    class func amountSpentInThePast(interval: String) -> [Double]
    {
        ...
        for _ in 1 ... size
        {
            ...
            if BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[key] == nil
            {
                BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[key] = 0.0
            }
            
            amountSpentArray.append(BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[key]!)
    ...
    class func logTodaysSpendings(num: Double)
    {
        ...
        let key = BigBudgetVariables.todaysDate(format: "MM/dd/YYYY")
        let monthKey = BigBudgetVariables.todaysDate(format: "MM/YYYY")
        ...
        if BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[monthKey] == nil
        {
            BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[monthKey] = 0.0
        }
        
        // Store the new amount into the dictionary with the key being today's date (MM/dd/YYYY)
        let newAmount = BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[key]! + num
        BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[key] = newAmount
        
        let newMonthAmount = BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[monthKey]! + num
        BigBudgetVariables.budgetArray[BigBudgetVariables.currentIndex].amountSpentOnDate[monthKey] = newMonthAmount
    }
    ...
    class func nameToNetAmtSpentMap() -> [String:Double]
    {
        ...
        if BigBudgetVariables.budgetArray.isEmpty == false
        {
            for i in 0...BigBudgetVariables.budgetArray.count - 1
            {
                nameToNetAmountMap[BigBudgetVariables.budgetArray[i].name!] = BigBudgetVariables.budgetArray[i].totalAmountSpent
            }
        }
    ...
    class func nameToTransactionCount() -> [String:Double]
    {
        ...
        if BigBudgetVariables.budgetArray.isEmpty == false
        {
            for i in 0...BigBudgetVariables.budgetArray.count - 1
            {
                nameToTransactionCountMap[BigBudgetVariables.budgetArray[i].name!] = Double(BigBudgetVariables.budgetArray[i].historyArray.count)
            }
        }
